...
- Ideally with launch automation is to automate everything. The key here is reliability. Reliability is the fundamental feature. And reliability comes from consistency. So in a self service release process, allows a high velocity teams create code, deploy simply automatically, and also control over critical operations
- Monitoring is measuring
+
image::Architecting with Google Compute Engine - Monitoring.png[Monitoring is measuring]
+
- Incident Response
...
** Develop a structured incident response including a dashboard, regimen, and plans and tools for responding to issues
** Have a balanced approach to the launch process
