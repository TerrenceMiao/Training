...
- Kubernetes uses **25 percent** of memory up to 4GB and then slightly less for the next 4GB, and it continues to reduce the percentage of additional memory down to **2 percent** of memory over 128GB
- Kubernetes takes **6 percent** CPU resources of the first core, down to **0.25 percent** of any cores above four cores
...
vulnerabilities
- Kubernetes deployment can be in states: **progressing**, **completed**, and **failed**
...
** The cost is based on machine type. The more CPUs and memory used, the higher the cost. You can create custom machine types
...
- gcloud compute instances create **--boot-disk-size** [BOOT DISK SIZE] **--boot-disk-type** [BOOT DISK TYPE] **--machine-type=n1-standard-4** **--preemptible** ...
...
- To add a GPU to an instance, you must start an instance in which GPU libraries have been installed or will be installed. Also verify that the instance will run in a zone that has GPUs available. And CPU must be compatible with the GPU selected, and GPUs cannot be attached to shared memory machines, and must set the instance to terminate during maintenance
- User with Compute Storage Admin role can create snapshots as backup
...
- Command line options: --flatten, **--format** (JSON format results), --verbosity, **--async** (causes information about the start process to be displayed), **--keep-disks=[all | boot]**, **--delete-disks=data**, --filter="zone:ZONE", e.g., gcloud compute instances delete ch06-instance-1 **–keep-disks=boot**
...
- Nodes execute the workloads run on the cluster. Nodes are VMs that run containers configured to run an application. Nodes are primarily controlled by the cluster master. The nodes run an agent called **kubelet**, which is the
...
- Ensure have a properly configured **kubeconfig** file, which contains information on how to communicate with the cluster API
...
- **gcloud components install app-engine-python** command will install or update the App Engine Python library as needed
...
- Autoscaling enables: **target_cpu_utilization**, **target_throughput_utilization**, **max_concurrent_requests**, **max_instances**, **min_instances**, **max_pending_latency**, **min_pending_latency**, **max_idle_instances**, **min_idle_instances**
...
** **max_idle_instances** / **min_idle_instances** specifies the maximum / mininum number of idle instances to be kept running and ready to serve traffic
- Basic scaling only allows parameters for **idle_timeout** and **max_instances**
- Manual scaling, then instances will be resident, and only allows parameter is **instances** (number of instances to run)
- **IP address**, **HTTP cookie** (preferred way), and **random splitting**, are allowed methods for splitting traffic
- The cookie used for splitting in App Engine is called **GOOGAPPUID**. It's a random value between 0–999
...
- **gcloud functions** breaks **gcloud [service] [resource] verb** command line convention
...
- Lifecycle rule can be specified on objects in Cloud Storage. Condition options: Age, Creation Data, Storage Class, Newer Versions, and Live State (live or archived versions of an object)
...
- A `Delete` action takes precedence over a `SetStorageClass` action. If multiple SetStorageClass actions are specified, the action switching to the storage class with the lowest at-rest storage pricing is chosen
...
- Nearline Storage or Coldline Storage objects have early deletion charges if they are overwritten less than 30 or 90 days from their creation time, respectively. If Object Versioning enabled for bucket, the original object remains in your bucket until it is explicitly deleted
- In a bucket with versioning enabled, deleting a live object will archive the object, while deleting an archived object will deletes the object permanently
- The **SetStorageClass** action incurs charges are determined by the original class of the object
- Early deletion and associated charges are based on the original creation time of the object, regardless of when the storage class changed. Rewriting time when Storage Class changed becomes the new creation time
...
- Updates to Cloud Storage Object lifecycle configuration may take up to 24 hours to go into effect
- The REST API returns the object’s expiration time in the `x-goog-expiration` response header
...
- By default buckets do not have lifecycle management enabled. To set or modify lifecycle configuration **ONLY** for an **existing bucket** must include an XML document in the request body that contains the lifecycle configuration
+
**Request**
+
[source.console]
...
- BigQuery datasets are stored at the region level, not globally or at the zone level
...
image::Associate Cloud Engineer Study Guide - BigQuery.png[BigQuery - Job history]
...
- `gsutil defstorageclass set [STORAGE_CLASS] gs://[BUCKET_NAME]` to set default storage class of an existing bucket
- `gsutil rewrite -s [STORAGE_CLASS] gs://[PATH_TO_OBJECT]` to change object Storage Class
- `gsutil mv gs://[SOURCE_BUCKET_NAME]/[SOURCE_OBJECT_NAME] gs://[DESTINATION_BUCKET_NAME]/[DESTINATION_OBJECT_NAME]`
- `gsutil mv gs://[BUCKET_NAME]/[OLD_OBJECT_NAME] gs://[BUCKET_NAME]/[NEW_OBJECT_NAME]`
- After created a Cloud Spanner instance, the next step is to create a database within the instance


Chapter 13 - Loading Data into Storage
--------------------------------------

- The first step in loading data into Cloud Storage is to create a bucket. From GCP Console, you can upload both files and folders
...
- **add-iam-policy-binding** parameter can work in **organizations**, **folders**, **projects** level in gcloud command
...
- **Hybrid Connectivity** section in Cloud Console to create a VPN
...
- `gcloud beta compute addresses delete [ADDRESS_NAME] --region [REGION]`
...
- In the GCP console there is an option between **From Internet To My VMs** and **Only Between My VMs**, this indicates public or private Load Balancer
...
- `gcloud compute forwarding-rules create my-int-lb-forwarding-rule --address my-ilb-ip-address --load-balancing-scheme internal --backend-service my-int-lb --ports 80 --region us-central1 --subnet my-subnet`
...
- **Machine type**, **Disk type**, **Disk size**, **Firewall** rules to allow both HTTP and HTTPS traffic, **Zone** can be configuired in Cloud Launcher
- Networking section in Cloud Launcher solution you will be able to configure IP Address, but not Access Controls. Can choose to have an ephemeral external IP or no external IP. External IP address makes site accessible from outside. Static IP is not an option. Also can specify source IP ranges for HTTP and HTTPS traffic
- Launch a solution by clicking the Launch on Compute Engine link in the **Overview page of the solution**
...
- Properties **deviceName**, **boot**, **autodelete** may be set when defining a disk on a virtual machine
