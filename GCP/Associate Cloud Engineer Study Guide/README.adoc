...
- Command line options: --flatten, **--format**, --verbosity, **--async** (causes information about the start process to be displayed), **--keep-disks=all**, **--delete-disks=data**, --filter="zone:ZONE", e.g., gcloud compute instances delete ch06-instance-1 **–keep-disks=boot**
...
- **gcloud beta container clusters create** ch07-cluster --num-nodes=3 --region=us-central1
...
- Networking section in Cloud Launcher solution you will be able to configure IP Address, but not Access Controls
