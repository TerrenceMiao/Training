...
- VPCs are logical data centers in the cloud. VPCs are global, they have subnets in all regions. Resources in any region can be accessed through the VPC
...
- The **Flow Log** option of command determines whether logs are sent to Stackdriver
...
- Firewall rules consist of direction (incoming / outcoming), priority (which of all the matching rules is applied), action (allow / deny), target, source/destination, protocols and port, and enforcement status. Firewall rules are applied to a subnet
- Compute and the resource used for creating a firewall rule
...
- If you choose policy-based routing, you will need to enter remote IP ranges network, local subnetworks that will use the VPN, and local IP ranges
