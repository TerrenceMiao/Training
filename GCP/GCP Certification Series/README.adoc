...
- To place a **lien** on a project protecting GCP project accidentally delete by any owners and administrator, until **lien** is revoked
...
- Organization nodes are optional, but must have one if want to define / create folders, or apply organization-wide policies centrally
+
...
** **Sustained use discount**: More your instances are running bigger discount you will get.
...
** **Rightsizing recommendations**: Google cloud provides recommendations for compute engine based on the **8 days Stackdriver monitoring** to resize the VM.
- Per-second billing
- Network traffic:
** Ingress is free
...
image::GCP Certification Series - Submarine Cable Map 2010.jpg[Submarine Cable Map 2010]

image::GCP Certification Series - Submarine Cable Map 2011.jpg[Submarine Cable Map 2011]
...
- Submarine Cable Map 2019, _https://submarine-cable-map-2019.telegeography.com/_
- Submarine Cable Map 2018, _https://submarine-cable-map-2018.telegeography.com/_
...
- VPC has TWO types of networks: **auto mode** network, **custom mode** network. You can switch a network from auto mode to custom mode. But custom mode network cannot be changed to auto mode network
...
+
[cols="5a,5a", frame=none, grid=none]
|===
|
...
Control resource permissions using a variety of options: graphically from the Google Cloud Platform console, programmatically via Cloud IAM methods, or using the gcloud command-line interface.
|
...
Leverage Cloud Identity, Google Cloud’s built-in managed identity to easily create or sync user accounts across applications and projects. It's easy to provision and manage users and groups, set up single sign-on, and configure multi-factor authentication directly from the Google Admin Console. You also get access to the GCP Organization, which enables you to centrally manage projects via Resource Manager.
|===
...
- User can be granted roles by **Cloud IAM policy**, which is a collection of statements that define who has what type of access, at level in the resource hierarchy. The **Cloud IAM policy** hierarchy follows the same path as the GCP resource hierarchy. The effective policy is the union of the policy set at specific level and the policy inherited from higher up in the hierarchy
** organization level
** folder level
** project level
** resource level
+
image::https://cloud.google.com/iam/img/iam-overview-policy.png[Cloud IAM Policy Overview]
+
[source.console]
----
{
...
*** Customised Service Account `SERVICE_ACCOUNT_NAME@PROJECT_ID.iam.gserviceaccount.com`
*** Can have up to 100 Service Accounts per project, including default Service Accounts
** Google-managed Service Accounts, created and owned by Google
*** Google API Service Account `PROJECT_NUMBER@cloudservices.gserviceaccount.com`
...
** **Application Default Credentials** are a mechanism to make it easy to use Service Accounts when operating inside and outside GCP, as well as across multiple GCP projects

** After deleting a Service Account, avoid creating a new Service Account with the SAME name. This can result in unexpected behavior
** Service Accounts are given a unique ID within Cloud IAM at creation. Internally, all role bindings are granted using these IDs, not the Service Account’s email address
...
**** when the Cloud Platform service does not provide a Predefined Role
...
- Identity Management product option
+
image::GCP Certification Series - IAM Product Options Flowchart.png[GCP Certification Series - IAM Product Options Flowchart]
+
** Cloud Identity for Customers and Partners (CICP)
+
image::https://storage.googleapis.com/gweb-cloudblog-publish/images/gcp_dash.max-1800x1800.png[Cloud Identity for Customers and Partners (CICP)]
+
...
image::https://cloud.google.com/logging/docs/images/life_of_a_log.png[Life of a log]
...
- Exported logs organisation
+
[source.console]
----
gcs-bucket/syslog/YYYY/MM/DD/
gcs-bucket/appengine.googleapis.com/request_log/YYYY/MM/DD/
----
+
The leaf directories contain multiple files, each of which holds the exported log entries for a time period specified in the file name. The files are sharded and their names end in a shard number, **Sn or An (n=0, 1, 2, ...)**. For example:
+
[source.console]
----
...
- fluentd, Unified Logging Layer, that runs on your virtual machine (VM) instances
+
image::https://miro.medium.com/max/2952/0*nDxKh2r-YOiPpLZS.png[How the Logging Agent works]


Commands
--------

image::gcloud command.jpeg[gcloud command]

image::gsutil command.jpeg[gsutil command]
...
** GCP Certification Series: 5.2 Managing service accounts, _https://medium.com/@prashantapaudel/gcp-certification-series-5-2-managing-service-accounts-2e7ea05ef7b9_
** GCP Certification Series: 5.3 Viewing audit logs for the project and managed services, _https://medium.com/@prashantapaudel/gcp-certification-series-5-3-viewing-audit-logs-for-the-project-and-managed-services-f4134ef636ee_
...
- Flowcharts
** A GCP flowchart a day, _https://medium.com/google-cloud/a-gcp-flowchart-a-day-2d57cc109401_
** More GCP flowcharts for your delectation, _https://medium.com/@grapesfrog/more-gcp-flowcharts-for-your-delectation-36b63ebb72ce_
** Some more GCP Flowcharts, _https://medium.com/google-cloud/some-more-gcp-flowcharts-dc0bb6f7c94e_
